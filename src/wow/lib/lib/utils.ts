...
export const supportGetDisplayMedia = (): boolean => {
  // @ts-ignore
  return navigator && (navigator.mediaDevices && !!navigator.mediaDevices.getDisplayMedia || !!navigator.getDisplayMedia)
}
 ...
 * @param constraints
 */
export const getUserMedia = async (constraints: MediaStreamConstraints, source: string): Promise<MediaStream> => {
  if (!navigator) {
  ...
  if (source === 'camera') {
    ...
    if (navigator.mediaDevices && navigator.mediaDevices.getDisplayMedia) {
      try {
        ...
        cnsl.error('Failed to getDisplayMedia(', error, ')')
        throw error
      }
    }
    return new Promise((resolve, reject) => {
      ...
      if (navigator.getDisplayMedia) {
        // @ts-ignore
        navigator.getDisplayMedia({video: true}).then(resolve).catch(reject)
      } else {
        reject(new Error('Your browser does not support getDisplayMedia API.'))
      }
    })
  }
}
 ...
 * @param constraints
 */
export const queryForCamera = async (constraints: MediaStreamConstraints, source: string): Promise<boolean> => {
  if (navigator && navigator.mediaDevices && navigator.mediaDevices.enumerateDevices) {
    const devices = await navigator.mediaDevices.enumerateDevices()
    return devices.filter(o => /video/.test(o.kind)).length > 0
  }
  const media = await getUserMedia(constraints, source)
  if (media) {
